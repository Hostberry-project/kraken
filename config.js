 ...
 */
function defaultMoriaDb() {
  if (process.env.PALANTIR_MORIA_DB) return process.env.PALANTIR_MORIA_DB
  if (process.env.FLY_APP_NAME) return '/data/moria.db'
  return ''
}
...
module.exports = {
  ...
  palantir: {
    ...
    moriaZm3: process.env.PALANTIR_MORIA_ZM3 || defaultMoriaZm3(),
    apiBase: process.env.PALANTIR_API_BASE || '',
    linksPath: process.env.PALANTIR_LINKS_PATH || '',
    authHeader: process.env.PALANTIR_AUTH_HEADER || 'Authorization',
    authPrefix: process.env.PALANTIR_AUTH_PREFIX ?? 'Bearer ',
    timeoutMs: Number(process.env.PALANTIR_TIMEOUT_MS) || 12000,
  },
