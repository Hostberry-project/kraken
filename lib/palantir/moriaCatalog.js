...
function likePattern(q) {
  return `%${normalizeSearchQuery(q).replace(/\s+/g, '%')}%`
}
...
function searchTable(db, table, type, query, skip = 0) {
  ...
  const sql = `
    ...
    WHERE titulo LIKE ? COLLATE NOCASE
       OR plot LIKE ? COLLATE NOCASE
    ORDER BY fecha DESC, titulo COLLATE NOCASE
    LIMIT ? OFFSET ?
  `
  return db.prepare(sql).all(like, like, PAGE_SIZE, skip)
}
...
function searchMoriaCatalog(type, extra = {}, userConfig = {}) {
  ...
  try {
    ...
    const q = String(extra.search || extra.query || '').trim()
    ...
    console.log(
      `[moria] catálogo ${type}/${table} q="${q || '(inicio)'}" skip=${skip} → ${metas.length}`
    )
