...
async function buildStreamsInner(type, stremioId, userConfig) {
  ...
  if (
    ...
    !hasEnabledPalantir(userOpts, userConfig)
  ) {
    const fallbacks = buildFallbackSearchStreams(type, stremioId, query)
    fallbacks.unshift({
      name: 'Kraken',
      title: 'Activa Peerflix/Torrentio/TorrentClaw, Palantir (API) o añade API debrid',
      externalUrl: 'https://real-debrid.com/apitoken',
    })
    return fallbacks
  }

  if (userConfig.enablePalantir !== 'false' && !isPalantirConfigured(userConfig)) {
    console.log(
      '[palantir] Activado en Stremio pero falta moria (PALANTIR_MORIA_DB) o API HTTP — ver README'
    )
  }
  ...
  const palantirPromise = userOpts.enablePalantir
  ...
  if (useFastPath) {
    ...
    if (palantirRaw.length) {
      console.log(`[Kraken] Palantir: ${palantirRaw.length} candidatos`)
    }

    const fromUpstream = upstreamToCandidates(
      upstreamStreams.filter((s) => !s.url)
    )
    let ranked = processCandidates([...raw, ...fromUpstream, ...palantirRaw], {
      strictSpanish: userOpts.strictSpanish,
    })
    ...
    const palantirOnly = ranked.filter((c) => c.source === 'Palantir')
    const debridPool =
  ...
  } else if (palantirRaw.length && userOpts.enablePalantir) {
    console.log(`[Kraken] Solo Palantir: ${palantirRaw.length} candidatos`)
    let ranked = processCandidates(palantirRaw, {
      strictSpanish: userOpts.strictSpanish,
    })
    if (hasAnyDebrid(services)) {
      const debridResults = await tryDebridList(
