...
async function waitForFiles(magnetId, apiKey, maxWaitMs = config.debridMagnetWaitMs || 14000) {
...
/** Enlace directo a archivo .torrent (muchas webs españolas) */
async function resolveTorrentUrl(torrentUrl, apiKey) {
  const uploaded = await apiGet(
    `/magnet/upload?links[]=${encodeURIComponent(torrentUrl)}`,
    apiKey
  )
  const magnetId = uploaded.magnets?.[0]?.id
  if (!magnetId) throw new Error('torrent no subido')
...
/** Hoster directo (1fichier, etc.) — mismo agent que Palantir 3 en Kodi */
async function resolveHosterLink(linkUrl, apiKey, agent = 'Palantir3') {
  const unlocked = await apiGet(
    `/link/unlock?agent=${encodeURIComponent(agent)}&link=${encodeURIComponent(linkUrl)}`,
    apiKey
  )
  if (!unlocked?.link) throw new Error('AllDebrid unlock sin enlace')
  return {
    url: unlocked.link,
    filename: unlocked.filename || 'video.mkv',
  }
}

module.exports = { resolveMagnet, resolveTorrentUrl, resolveHosterLink }
