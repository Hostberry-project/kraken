...
|---------|----------------|
| **kraken-stremio** (activa) | `https://kraken-stremio.fly.dev/manifest.json` |
| kraken-project | `https://kraken-project.fly.dev/manifest.json` (solo si responde) |

Usa **kraken-stremio** salvo que hayas desplegado y probado `kraken-project`.
...
```bash
fly volume create moria_data -a kraken-stremio -r ams -n 2 --size 2
```

Luego: `fly deploy --config fly.toml -a kraken-stremio` o push a `main`.

### Error `read ./: is a directory`

Ocurre si en el panel de Fly usas **Launch** con `--config ./`. **No uses Launch** para esta app: ya existe `kraken-stremio`. Despliega con GitHub Actions o:

```bash
cd ruta/al/repo
fly deploy --config fly.toml -a kraken-stremio
```
